...
void write(uint16_t addr, uint8_t value)
{
	...
	{
		// TODO: Mapper register wiriting implementation.
		/*switch (GET_MAPPER(&hdr))
		{
			case 2:
				bank_switch_16k(value & 0x0F, ROM, image);
				break;
			case 0:
			default:
		...
		printf("p6502 Write val $%02x to addr : $%04x\n", value, addr);
		if (addr == 0xEAEA);
			finished = 1;
	}
}
...
void reset(pP6502 p)
{
	...
	p->p = FLAG_1 | Z_FLAG;			// Bit 5 is always 1, and interrupt mask is set
...
void fetch(pP6502 p)
{
	...
	if (p->interrupts != INTR_NONE)
	{
		...
		else if ((p->interrupts & INTR_IRQ) && (!(p->p & I_FLAG)))		// IRQ
		{
			...
			PUSH(p->p | I_FLAG);		// Disable interrupts.
	...
	switch (instruction)
	{
		case 0x69:										 // ADC #Oper - Immadiate
			...
			//GET_IMMEDIATE(result);
			//src = result.b.l;
			//result.w = p->a + src + (p->p & C_FLAG);
			//p->p &= ~(N_FLAG | Z_FLAG | C_FLAG | V_FLAG);
			//p->p |= (result.b.l & N_FLAG)
			//		| ((~(p->a ^ src) & (p->a ^ result.b.l) & 0x80) ? V_FLAG : 0)
			//		| (result.b.h ? C_FLAG : 0) | (result.b.l ? 0 : Z_FLAG);
			//p->a = result.b.l;
		...
		case 0x00:												// BRK
			...
			p->p = (p->p | I_FLAG) & ~D_FLAG;		// Disable interrupt.
		...
		//case 0x08: PUSH(p->p);break;							// PHP - Implied
		case 0x08: PUSH(p->p | FLAG_1 | B_FLAG); break;			// X6502 compatiblity
		case 0x68: POP(p->a); FLAG_N_Z(p->a);break;				// PLA - Implied
		//case 0x28: POP(p->p); p->p |= B_FLAG | FLAG_1; break;	// PLP - Implied
		case 0x28: POP(p->p); break;							// X6502 compatibility
		...
		default:
			...
			printf("\nUndefined opcode at 0x%04X : 0x%02X at cc : %d\n", p->pc.w - 1, instruction, p->cycle_counter);
			break;
	}
