...
int PPU_Init(pPpuStatus ppu)
{
	...
	if (!w) {
		DebugMessage("SDL_CreateWindow() failed!");
		return 0;
	}
	...
	if (SDL_SetWindowDisplayMode(w, &mode) < 0) {
		DebugMessage("Setting display mode failed.!");
		return 0;
	}

	s = SDL_GetWindowSurface(w);
	DebugMessage("Byte per pixel : %d = '%s'   ", s->format->BytesPerPixel,
...
void PPU_Write(pPpuStatus ppu, uint16_t addr, uint8_t value)
{
	switch (addr & 0x2007) {
		case PPU_CTRL_REG:		// $2000
			LogInfo("writing $2000 (PPU CTRL REG)      : 0x%02X at cc:%d", value, p.cycle_counter);
			ppu->regs.ctrl = value;
			ppu->t &= ~0x0C00;
		...
		case PPU_MASK_REG:		// $2001
			LogInfo("writing $2001 (PPU MASK REG)      : 0x%02X at cc:%d", value, p.cycle_counter);
			ppu->regs.mask = value;
			break;

		case OAM_ADDR_REG:		// $2003
			LogInfo("writing $2003 (OAM ADDR REG)      : 0x%02X at cc:%d", value, p.cycle_counter);
			ppu->regs.oam_address = value;
			break;

		case OAM_DATA_REG:		// $2004
			LogInfo("writing $2004 (OAM DATA REG)      : 0x%02X at 0x%02X at cc:%d", value, ppu->regs.oam_address, p.cycle_counter);
			ppu->OAM.u.memory[ppu->regs.oam_address++] = value;
			break;

		case PPU_SCROLL_REG:		// $2005
			LogInfo("%s writing $2005 (PPU SCROLL REG) : 0x%02X at cc:%d", (ppu->w ? "second" : "first"), value, p.cycle_counter);
			...
			LogInfo("                    x : 0x%02X       t : 0x%04X      v : 0x%04X at cc:%d", ppu->x, ppu->t, ppu->v, p.cycle_counter);
			break;

		case PPU_ADDR_REG:		// $2006
			LogInfo("%s writing $2006 (PPU ADDR REG)   : 0x%02X    v(addr) : 0x%04X          t : 0x%04X at cc:%d", (ppu->w ? "second" : "first "), value, ppu->v, ppu->t, p.cycle_counter);
			if (ppu->w == 0) {		// First write
				...
				ppu->v = ppu->t;
				ppu->w = 0;
				PPU_DebugNameTable(ppu, GREEN_COLOR);
			}
			LogInfo("             after writing addr :       v : 0x%04x          t : 0x%04X at cc:%d", ppu->v, ppu->t, p.cycle_counter);
			break;

		case PPU_DATA_REG:		// $2007
			LogInfo("writing $2007 (PPU DATA REG) 0x%02X at 0x%04X at cc:%d", value, ppu->v, p.cycle_counter);
#if defined _WIN32 && !defined __GNUC__
...
#endif
			{
				if (!Mapper_WritePPU(addr, value))				// Mapper 2 hase CHAR RAM.
					LogError("WRONG PPU ADDRESS! Writting 0x%02X at 0x%04X to pattern ROM!", value, ppu->v);
				...
				switch (mirroring) {
					...
					default:
						LogError("Unknown mirroring type : %d", mirroring);
						break;
				}
		...
		default:
			LogError("writing 0x%02X at 0x%04X", value, ppu->v);
			break;
	}
}
...
uint8_t PPU_Read(pPpuStatus ppu, uint16_t addr)
{
	...
	switch (addr) {
		case PPU_STATUS_REG:		// $2002
			LogInfo("reading $2002 (PPU STATUS REG) 0x%02X at cc:%d", ppu->regs.status, p.cycle_counter);
		...
		case OAM_DATA_REG:		// $2004
			LogInfo("reading $2004 (OAM DATA REG)   0x%02X from 0x%02X at cc:%d", ppu->OAM.u.memory[ppu->regs.oam_address], ppu->regs.oam_address, p.cycle_counter);
			return ppu->OAM.u.memory[ppu->regs.oam_address];

		case PPU_DATA_REG:		// $2007
			LogInfo("reading $2007 (PPU DATA REG)   0x%02X at cc:%d", ppu->regs.data, p.cycle_counter);
		...
		default:
			LogError("reading at 0x%04X at cc:%d", (int)ppu->v, p.cycle_counter);
			break;
	}

	LogError("Invalid PPU register read at address : 0x%04X at cc:%d", addr, p.cycle_counter);
	return 0;
}
...
void PPU_Clock(pPpuStatus ppu)
{
	...
	if (ppu->scanline == SCANLINE_VINT_START && ppu->clock_cycle == 0) {
		...
		PPU_DebugNameTable(ppu, BLUE_COLOR);
		PPU_DebugPatternTable(ppu);
		LogInfo("Start of frame    t: $%04x     v: $%04x at cc:%d", ppu->t, ppu->v, p.cycle_counter);
	...
	if ((++ppu->clock_cycle) == SCANLINE_CLOCK_END) {
		...
		if ((++ppu->scanline) == SCANLINE_NUMBER_END) {
			...
			PPU_DebugNameTable(ppu, RED_COLOR);
			LogInfo("End of frame      t: $%04x     v: $%04x at cc:%d\n", ppu->t, ppu->v, p.cycle_counter);
		}
	}
}
	...
	for (x = 0; x < 32 * 30; ++x) {
		...
		for (y = 0; y < 8; ++y) {
			...
			if (nt == 1)
				...
				idx += 30 * 8 * WINDOW_NAME_TABLE_WIDTH;
			else if (nt == 3)
				idx += 30 * 8 * WINDOW_NAME_TABLE_WIDTH + 256;
...
void PPU_DebugNameTable(pPpuStatus ppu, uint32_t color)
{
	...
	PlotVerticalLine(ppu, ((COARSE_X(ppu->v) << 3) + ppu->x), color);
	PlotHorizontalLine(ppu, COARSE_Y(ppu->v), color);
